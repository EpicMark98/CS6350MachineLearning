...
def load_examples(A, names, filename = "train.csv"):
    dataset = []
    with open(filename, 'r') as f:
        ...
        for line in f:
            ...
            for i in range(len(stringArray)):
                x = stringArray[i]
                if isnumeric(x):
                    currItem = np.append(currItem, float(x))
                else:
                    try:
                        attributeName = names[i]
                        possibleValues = A[attributeName]
                        currItem = np.append(currItem, float(possibleValues.index(x)))
                    except:
                        currItem = np.append(currItem, float(0))
            currItem = np.insert(currItem, 1, 1)
...
def get_prediction_voted(x, w_list, count_list):
    ...
    for i in range(len(w_list)):
        totalVote += count_list[i] * get_prediction(x[1:], w_list[i])
    totalVote /= total
    totalVote = (totalVote + 1) / 2
    return str(totalVote)
